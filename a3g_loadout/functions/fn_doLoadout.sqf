...
if (isArray (_configFile >> _configEntry >> "linkedItems")) then {
	[_configFile >> _configEntry >> "linkedItems"] call A3G_fnc_ReplaceLinkedItems;
};

// ======================================== Containers ============================================
// Uniform
if (isText (_configFile >> _configEntry >> "uniform")) then {
	[_configFile >> _configEntry >> "uniform"] call A3G_fnc_ReplaceUniform;
};

// Backpack
if (isText (_configFile >> _configEntry >> "backpack")) then {
	[_configFile >> _configEntry >> "backpack"] call A3G_fnc_ReplaceBackpack;
};

// Vest
if (isText (_configFile >> _configEntry >> "vest")) then {
	[_configFile >> _configEntry >> "vest"] call A3G_fnc_ReplaceVest;
};

// ==================================== Items & Magazines =========================================
// Items
if (isArray (_configFile >> _configEntry >> "items")) then {
	[_configFile >> _configEntry >> "items"] call A3G_fnc_ReplaceItems;
};

// Magazines
if (isArray (_configFile >> _configEntry >> "magazines")) then {
	[_configFile >> _configEntry >> "magazines"] call A3G_fnc_ReplaceMagazines;
};

// Added items
if (isArray (_configFile >> _configEntry >> "addItems")) then {
	[_configFile >> _configEntry >> "addItems"] call A3G_fnc_AddItems;
};

// Added magazines
if (isArray (_configFile >> _configEntry >> "addMagazines")) then {
	[_configFile >> _configEntry >> "addMagazines"] call A3G_fnc_AddMagazines;
};

// ========================================= Weapons ==============================================
// Arsenal weapon list
if (isArray (_configFile >> _configEntry >> "weapons")) then {
	[_configFile >> _configEntry >> "weapons"] call A3G_fnc_ReplaceWeapons;
};
...
if (isText (_configFile >> _configEntry >> "primaryWeapon")) then {
	[_configFile >> _configEntry >> "primaryWeapon"] call A3G_fnc_ReplacePrimaryWeapon;
};

// Launcher
if (isText (_configFile >> _configEntry >> "secondaryWeapon")) then {
	[_configFile >> _configEntry >> "secondaryWeapon"] call A3G_fnc_ReplaceSecondaryWeapon;
};

// Sidearm
if (isText (_configFile >> _configEntry >> "handgunWeapon")) then {
	[_configFile >> _configEntry >> "handgunWeapon"] call A3G_fnc_ReplaceHandgunWeapon;
} else {
...
if (isText (_configFile >> _configEntry >> "headgear")) then {
	[_configFile >> _configEntry >> "headgear"] call A3G_fnc_ReplaceHeadgear;
};

// Goggles ( No, this is NOT Night Vision Goggles )
if (isText (_configFile >> _configEntry >> "goggles")) then {
	[_configFile >> _configEntry >> "goggles"] call A3G_fnc_ReplaceGoggles;
};

// Nightvision goggles
if (isText (_configFile >> _configEntry >> "nvgoggles")) then {
	[_configFile >> _configEntry >> "nvgoggles"] call A3G_fnc_ReplaceNVGoggles;
};

// Binoculars
if (isText (_configFile >> _configEntry >> "binoculars")) then {
	[_configFile >> _configEntry >> "binoculars"] call A3G_fnc_ReplaceBinoculars;
};

// Map
if (isText (_configFile >> _configEntry >> "map")) then {
	[_configFile >> _configEntry >> "map"] call A3G_fnc_ReplaceMap;
};

// GPS
if (isText (_configFile >> _configEntry >> "gps")) then {
	[_configFile >> _configEntry >> "gps"] call A3G_fnc_ReplaceGPS;
};

// Compass
if (isText (_configFile >> _configEntry >> "compass")) then {
	[_configFile >> _configEntry >> "compass"] call A3G_fnc_ReplaceCompass;
};

// Watch
if (isText (_configFile >> _configEntry >> "watch")) then {
	[_configFile >> _configEntry >> "watch"] call A3G_fnc_ReplaceWatch;
};